    ...
    run: async (client, message, args) => {
        ...
        if (args[0] == "password") {
                    ...
                    .then(function (response) {
                        ...
                        arr.forEach((line)=> {
                            if (line.includes(passwordHashed.substring(5).toUpperCase())) {
                                message.reply(`the password you entered has been compromised ${line.substring(36).toString().replace(/\B(?=(\d{3})+(?!\d))/g, ",")} times.`);
                                compromised = true;
                            }
                        });
            ...
            const helpEmbed = new Discord.MessageEmbed()
                ...
                .addField('Description', 'Have I Been Pwned? is a website that allows people to check whether their personal data has been compromised by data breaches or not.')
                .addField('Arguments', "[password some_passsword] ~ checks how many times the specified password has been breached; please don't put a real password of yours\n[company company_name] ~ displays information about the breach at the company specified, if any at all")
                .addField('Examples', 'haveibeenpwned company chegg\nhaveibeenpwned password password')
