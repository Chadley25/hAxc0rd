module.exports = {
    name: "virustotal",
    category: "recon",
    description: "Uses the VirusTotal API to check if a website or file is infected with malware",
    run: async (client, message, args) => {
        ...
        if (args[0] == "url") {
            ...
            // checks if the file contains an attatchment
        ...
        function virusTotalURLCheck(link, title) {
            // sends a get request to the VirusTotal API with an argument of an URL
            // and parses through the returned data and displays to the user which
            // programs detected a positive result from the URL and which ones didn't
            axios.get(link)
                .then(function (response) {
                    let cleanDetect = "";
                    let malwareDetect = "";
